...
const distributeCards = async (room, roomName) => {
    ...
    await PLAYER_KEYS.forEach(async (key) => {
        if (room[key] && !room[key].isWaiting) {
            room[key].cardFirst = arr[index];
            room[key].cardSecond = arr[index + 1];
            room[key].cardThird = arr[index + 2];
            room[key].flipCardFirst = false;
            room[key].flipCardSecond = false;
            room[key].flipCardThird = false;
            index += 3;
...
const updateCoin = async (roomSocket, room) => {
    ...
    await UserModel.find({ userName: room.ownerOfRoom.userName })
        .then(async (users) => {
                ...
                if (room[key] && key !== 'ownerOfRoom') {
                    await UserModel.find({ userName: room[key].userName })
                        .then(async (users) => {
                            ...
                            await UserModel.findOneAndUpdate({ userName: room[key].userName }, {
            ...
            await UserModel.findOneAndUpdate({ userName: room.ownerOfRoom.userName }, {
